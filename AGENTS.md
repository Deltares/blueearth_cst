...
- `config/` — four bins: `workflows/` (`snake_config_*.yml`, the `--configfile`
  targets — but NOT the baseline seed, which lives at
  `test_case/snake_config_model_test.yml`, beside the project it writes into, and
  is the one tracked file under the otherwise-ignored `test_case/`; the
  `.gitignore` entry is `test_case/*` plus a negation for exactly that reason,
  since git cannot un-ignore a file whose parent DIRECTORY is excluded.
  `workflows/archive/` holds unmaintained single-workflow configs),
