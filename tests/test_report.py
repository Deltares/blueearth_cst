...
CLEAN_RUN = {
    ...
    "sources": [
        {
            "reference_window_effective": "1990-01-01 / 2010-12-01",
...
def test_a_clipped_window_says_so():
    doc = dict(CLEAN_RUN)
    doc["reference_window"] = dict(
        CLEAN_RUN["reference_window"], reference_window_clipped=True
    )
...
def test_a_short_window_is_called_out():
    ...
    doc["sources"] = [
        {
            "reference_window_effective": "1996-01-01 / 2010-12-01",
...
def test_matching_alignment_says_matches():
    doc = dict(CLEAN_RUN)
    doc["reference_window"] = dict(
        CLEAN_RUN["reference_window"], reference_alignment="matches"
    )
