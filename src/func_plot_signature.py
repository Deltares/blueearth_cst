...
def plot_signatures(dsq, labels, colors, linestyles, markers, Folder_out, station_name, lw = 0.8, fs = 8):
    ...
    for label in labels:
        ...
        #nse logq
        nselog = skills.lognashsutcliffe(dsq['Q'].sel(runs=label), dsq['Q'].sel(runs='Obs.'))
...
#    P_sum_monthly_mean.to_series().plot.bar(ax=ax1, color = 'lightblue')
    ...
    T_mean_monthly_q25 = ds_clim['T_subcatchment'].groupby('time.month').quantile(0.25, 'time')
    T_mean_monthly_q75 = ds_clim['T_subcatchment'].groupby('time.month').quantile(0.75, 'time')
    #plot
