...
def pareto_pair(results_file, comb, plot_folder, soil_params, show=False):
    ...
    data_pair = results_file[[m1, m2] + soil_params]
    ...
    pareto_front_data = pareto_front_data.join(results_file[soil_params])
    
    # List of parameters to color by (including euclidean)
    color_params = ['euclidean'] + soil_params
...
def main(calib_dir, observed, out_interactive):
    ...
    #PARETO
    for comb in combs:
        pareto_pair(results_file, comb, plot_folder, soil_params, show=True)
...
if __name__ == "__main__":
    ...
    if "snakemake" in globals():
        ...
        main(snakemake.params.calib_folder, 
