...
#add station_name
qsim_gauges = qsim_gauges.assign_coords(station_name = ("index", ['wflow_' + x for x in list(qsim_gauges["index"].values.astype(str))])) 
...
if f"{gauges_output_name}" in mod.staticgeoms:
    ...
    #add station_name
    qsim_outloc = qsim_outloc.assign_coords(station_name = ("index", list(gdf_outlocs['station_name'][qsim_outloc.index.values].values))) 
    #rename run to label and rename var to Q
    ds_sim_outlocs = qsim_outloc.to_dataset().assign_coords({"runs" : labels[0]}).expand_dims("runs").rename({f"Q_{gauges_output_name}":"Q"})

#P, EP and T for wflow_subcatch
ds_clim = xr.merge([mod.results["P_subcatchment"],
...
if (f"{gauges_output_name}" in mod.staticgeoms) & (os.path.exists(observations_timeseries)):
    name = f"Q_{gauges_output_name}"  # gauges locations in staticgeoms
    da_ts = hydromt.io.open_timeseries_from_table(observations_timeseries, name=name, sep=";")
    da = hydromt.vector.GeoDataArray.from_gdf(gdf_outlocs, da_ts, index_dim="index")
    qobs_outloc = da
    #rename run to Obs. and rename var to Q
    ds_obs = qobs_outloc.to_dataset().assign_coords({"runs" : "Obs."}).expand_dims("runs").rename({f"Q_{gauges_output_name}":"Q"})
...
ds_sim_gauges = qsim_gauges.to_dataset().assign_coords({"runs" : labels[0]}).expand_dims("runs").rename({"Q_gauges":"Q"})
    
#select dataset based on gauges or/and outputloc locations    
#if no user output and observations are provided:
if ((f"{gauges_output_name}" in mod.staticgeoms) == False) & (os.path.exists(observations_timeseries)==False):
    ds_list = [ds_sim_gauges]
#if user output locs are available but no observations timeseries:
elif ((f"{gauges_output_name}" in mod.staticgeoms) == True) & (os.path.exists(observations_timeseries)==False):
...
for ds in ds_list:
    for station_id, station_name in zip(ds.index.values, ds.station_name.values):
        ...
        if len(ds.time) > 365: 
            dsq = ds.sel(index = station_id).sel(time = slice(f"{ds['time.year'][0].values+1}-{ds['time.month'][0].values}-{ds['time.day'][0].values}", None))#.dropna(dim='time')
        else:
        ...
        if len(np.unique(dsq['time.year'])) >= 3:
            ...
            if year_min == year_max:
                year_min = dsq.resample(time = 'A').min('time').isel(time = 1)['time.year'].values
...
for index in ds_clim.index.values:
    ...
    ds_clim_i = ds_clim[['P_subcatchment', 'EP_subcatchment', 'T_subcatchment']].sel(index = index)
