...
def get_upstream_clim_basin(
    ...
    #add area 
    qobs = qobs.assign_coords(area=("index", list(gdf_basins["area"][qobs.index.values].values)))
    ...
    #annual sum
    ds_clim_sub_annual = ds_clim_sub[["specific_Q", "precip_mean", "pet_mean"]].resample(time=f"YS-{start_hydro_year}").sum(
...
def plot_budyko(
    ...
    plt.plot(xxx, 1 - 1 / (1 + xxx**(-omega))**(1/omega), color = "k", linewidth = 1.2, label = "Budyko")
    #energy limit
