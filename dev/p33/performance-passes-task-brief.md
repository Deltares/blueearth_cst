...
> is `dev/p33/performance-passes-design.md` (ACCEPTED 2026-07-24, v4,
...
> `dev/p33/performance-passes-design-review-record.md`; scoping anchors:
> `dev/p33/performance-passes-intake.md`; probe evidence `dev/p33/probes/`.
...
- **Key design pins — implement exactly:** rule 3.10 restructure to
  ...
  construct, `dev/p33/probes/snakemake-output-expressibility/`); the
...
`dev/scripts/estimate_batch_makespan.py` (new); `dev/p33/**` (baseline
...
1. `p33: record wf3 performance baseline + cost decomposition` — land
   ...
   write the durable baseline note under `dev/p33/` (à la baseline_diffs:
