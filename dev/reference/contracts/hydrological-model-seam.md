...
- **path pattern:** `models/hydrology/wflow/forcing/inmaps_historical.nc` (wf1
  forcing); wf3 twin `<exp>/hydrology/wflow/rlz_<n>/forcing/inmaps_cst_<m>.nc`
...
- **path pattern:** `models/hydrology/wflow/wflow_sbm.toml` (base) and per-cst
  `<exp>/hydrology/wflow/rlz_<n>/config/cst_<m>.toml`. R07 B5 moved the run
...
- **path pattern:** wf1 `models/hydrology/wflow/run_default/output.csv`; wf3
  `<exp>/hydrology/wflow/rlz_<n>/output/cst_<m>.csv` (R07 B5: the realization
...
- **path pattern:** `<exp>/hydrology/wflow/rlz_<n>/output/outstates_cst_<m>.nc`.
...
| `validate_hm2` | HM-2 (+ WG-6 twin) | `models/hydrology/wflow/forcing/inmaps_historical.nc`; wf3 twin `<exp>/hydrology/wflow/rlz_<n>/forcing/inmaps_cst_<m>.nc` | **yes** for wf1 `inmaps_historical.nc`; wf3 twin (WG-6) `temp()` → skip-until-captured |
| `validate_hm3` | HM-3 | `models/hydrology/wflow/staticgeoms/{region.geojson, outlets.geojson, outlet_index.csv}` | **yes** (persists) |
| `validate_hm4` | HM-4 | `models/hydrology/wflow/wflow_sbm.toml`; `<exp>/hydrology/wflow/rlz_<n>/config/cst_<m>.toml` | **yes** (both base + per-cst TOMLs persist) |
| `validate_hm5` | HM-5 | wf1 `run_default/output.csv`; wf3 `<exp>/hydrology/wflow/rlz_<n>/output/cst_<m>.csv` | **yes** (both persist; the wf3 per-cst CSVs are NOT `temp()`) |
...
| `validate_hm6b` | HM-6b | `<exp>/hydrology/wflow/rlz_<n>/output/outstates_cst_<m>.nc` | **no** — `temp()` content absent; skip-until-captured on disk, synthetic-proven every suite |
| `validate_hm7` | HM-7 | `<exp>/indicators/{q_indicators.csv, basin.csv}` | **yes** (persists; `rule all`, manifested) |
...
|---|---|---|
| `validate_hm6b` | HM-6b wf3 warm state NC | `<exp>/hydrology/wflow/rlz_<n>/output/outstates_cst_<m>.nc` |
