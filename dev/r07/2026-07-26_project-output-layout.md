...
> **SUPERSEDED 2026-07-26 by `dev/r07/project-layout-design.md`** (with the path
> map at `dev/r07/migration_project-layout.md`), which consolidates this note and
...
> `dev/p31/experiment-structure-design.md:287-313`.
...
between them (`dev/p32a/climate-analysis-design.md:385,407`, ext1-5) — a
...
(`dev/p31/migration_experiment-structure.md` §2). It is a byte-identical duplicate
...
2. Write the `dev/<milestone>/migration_<topic>.md` old→new path map required by
   ...
   as `dev/p31/migration_experiment-structure.md`.
