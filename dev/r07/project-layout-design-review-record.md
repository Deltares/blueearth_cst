...
**The audit trail behind `dev/r07/project-layout-design.md` (ACCEPTED 2026-07-28).**
...
`dev/r07/project-layout-intake.md`.
...
- **risk-1** finds the design reverses a *prior accepted design* without citing it:
  `dev/p32a/climate-analysis-design.md:389-405` rejects the single store
...
- **arch-2** cites the derived migration map (`dev/r07/migration_project-layout.md:154`,
...
`dev/p32a/climate-analysis-design.md:380-388` records an empirical `allclose`
...
findings:
  - id: risk-1
    ...
    finding: >-
      ...
      not in `## Alternatives considered`. `dev/p32a/climate-analysis-design.md:389-405`
    ...
    suggested_fix: >-
      Name the producer explicitly in B1 and rebut `dev/p32a/climate-analysis-design.md`
  ...
  - id: risk-5
    ...
    finding: >-
      ...
      **outside** the repository and is therefore covered by no `.gitignore` and no
      cleanup. `MIGRATION.md` and `dev/r07/migration_project-layout.md` are both
  ...
  - id: risk-8
    ...
    finding: >-
      ...
      the design duly files its map at `dev/r07/migration_project-layout.md`. But by the
    ...
    suggested_fix: >-
      ...
      `project_dir` — go to `docs/`. Then say which class `dev/r07/migration_project-layout.md`
...
- **Grid equality under B1 is *not* a finding.** I checked it and the design is
  ...
  and `dev/p32a/climate-analysis-design.md:380-388` records an empirical `allclose`
...
findings:
  ...
  - id: arch-2
    ...
    finding: >-
      ...
      (`dev/r07/migration_project-layout.md:154`, "**MISSING** — none by design"), cited here
  ...
  - id: arch-11
    ...
    finding: >-
      ...
      `dev/<milestone>/` — and R07 writes its own map to `dev/r07/migration_project-layout.md`,
...
findings:
  ...
  - id: repo-6
    ...
    suggested_fix: >-
      ...
      the corresponding rows to `dev/r07/migration_project-layout.md`'s config-path
  ...
  - id: repo-11
    ...
    finding: >-
      ...
      `dev/<milestone>/migration_<topic>.md` the required artifact of a contract
      rename. Moving the R06 note to `docs/` does not create a `dev/r06/` note, so
      §7 stays unsatisfied for R06 unless §7 itself is amended. (b) After the move
      the repo has `docs/MIGRATION.md` (R06) and
      `dev/r07/migration_project-layout.md` (R07) — two consecutive milestones,
...
name; and `dev/r07/migration_project-layout.md` mixes a snake_case prefix with a
...
with no `dev/r06/` note at all, and the R06/R07 pair would sit in two locations
...
|---|---|---|---|---|---|
| risk-1 | internal-panel | blocking | accepted | B1 now names the producer: one rule definition over region + catalog, declared in both Snakefiles (owner ruling GA-1). **Neither of risk-1's own fixes adopted** — (i) pulling a standalone extraction rule into R07 and (ii) deferring B1 were both rejected at G1-return; alternatives now record why. The finding's second demand *is* met: B1 carries a point-by-point rebuttal table against `dev/p32a/climate-analysis-design.md` §"Why a separate wf1 extraction", which v1 engaged nowhere, and the scope-authority header lists p32a as explicitly superseded. | design-v2.md |
