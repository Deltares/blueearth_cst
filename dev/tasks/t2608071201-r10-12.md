...
**Landed 2026-08-08 (R11 P3):** `README.rst`, *Running* → "Re-running an
experiment after a model rebuild". It states that the refusal is expected after
any WF1 rebuild, splits the response by what the error names (forcing alone →
re-record; anything else → the guard found something real, create a new
experiment), and carries this item's cost sentence so the trade-off stays visible
where an operator reads it rather than only on the board.
