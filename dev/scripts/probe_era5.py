...
print(
    f"Min: {np.nanmin(t2m):.3f}  Max: {np.nanmax(t2m):.3f}  Mean: {np.nanmean(t2m):.3f}"
)
print(
    f"All-NaN per timestep count: {int(np.all(np.isnan(t2m), axis=(1, 2)).sum())} / {t2m.shape[0]}"
)
...
print(
    f"  time[0]: {str(dst['time'].values[0])[:19]}  time[-1]: {str(dst['time'].values[-1])[:19]}"
)
