...
| [`basin_map_example.py`](basin_map_example.py) | A worked, **editable** example of calling the layer-in `plot_basin_map(dem, rivers, basin, ...)` directly. Three numbered blocks at the top of the file are the whole interface: which optional layers to draw, which columns they are read through (`gauge_label_column`, `river_order_column`, `extent`), and any style constants to override. Edit, re-run, look. Use it to see what the function TAKES — to drop a layer, re-frame the map, or work out how to plot a basin that never came from wflow; use `preview_basin_map.py` instead to tune or sweep a style constant from the command line. Writes one PNG to a gitignored scratch dir. |
