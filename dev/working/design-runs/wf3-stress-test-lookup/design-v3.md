# WF3 — stress-test lookup and derived response-surface axes: Design (v3, DRAFT)

> **Status: DRAFT (v3)** — stage 6 (revision r2) of the `design-review-loop` run
...
> (both blocking — `architecture-1`, `repo-fit-1` — closed by §5.7); **v3 answers
> external round 1's six** (`external-review-r1.md`), whose two blocking findings
> are the caption's false annual label (`ext1-1`, closed in §5.5/§5.8 by **D31**)
> and the notebook contradicting the settled no-caller ruling (`ext1-2`, closed in
> §8 step 6). All 32 are dispositioned one row each in `ledger.md`, which remains
...
> **Size budget for the normative body:** **raised at v3 from 2,150 to 2,600
> lines** (v2 raised it from 1,250 to 2,150 and landed at 2,133; v3 is 2,615).
> Raised rather than exceeded silently, for the same reason as at v2. Where v3's
...
2. **`join_axes` asserts its partition and raises `BaselinePartitionError`**
   ...
   **exactly** the baseline token `"0".zfill(key_width)`, and `surface_df` must
...
>
> An implementation then **asserts its partition**: the set of `st_id` present in
> the tables but absent from the lookup must be **exactly** the baseline token, and
> the surface partition must be non-empty. Either violated is an error, not a
> shape.
>
...
2. **The reconstructed FORCING moves by at most one `float64` ulp of the
   perturbation level — and on every shipped config, not at all.** v1 claimed the
...
- **Step 4 — the library and its tests.** `shared/surface_axes.py` (with D28's
  ...
  an ungated file. V5–V11 and V18–V19 map onto named functions there (§9), and the
...
| **V20** | **A non-round grid's forcing is reconstructed within the stated bound** — a claim about the MULTIPLIER only, and about no indicator (§7-2, R13) | reconstructed multiplier differing from the `float32` level by more than one `float64` ulp on a 0.6–1.4 `step_num: 3` grid (D25) — the case the shipped configs cannot exercise | `test_prepare_cst_parameters.py` |
