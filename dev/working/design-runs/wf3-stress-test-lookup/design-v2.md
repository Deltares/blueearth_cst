# WF3 — stress-test lookup and derived response-surface axes: Design (v2, DRAFT)

> **Status: DRAFT (v2)** — stage 3 (revision r1) of the `design-review-loop` run
> `wf3-stress-test-lookup`. Not accepted; G1 (framing) is **approved**, with a
> **G1-return** ruling two forks after the internal panel; G2 (design) is
> `pending` (`status.md`).
> **Revision basis:** the 26 internal-panel findings, dispositioned one row each
> in `ledger.md` (the run record for per-finding argument). Both blocking
> findings — `architecture-1`, `repo-fit-1` — are closed by §5.7.
...
> **Size budget for the normative body:** **raised at v2 from 1,250 to 2,150
> lines**; v2 is 2,133. Stated as a figure and a reason rather than quietly
> exceeded, because the original was set against this repo's accepted designs
...
**The bound, stated normatively.** After migration the multiplier the generator
receives differs from the pre-migration multiplier by **at most one `float64` ulp
of the level** (relative ~2.2e-16), and is bit-identical for the majority of
levels including every level in every shipped config. Worked counter-examples,
measured over `np.linspace`:
...
corrected: a failing group means the forcing moved **by more than one ulp**, which
D25's arithmetic cannot explain and a lost quantization discipline can.

Two things follow for §7. The consequence "indicator values do not move" is
restated as **within the baseline comparator's tolerance**, not bit-identically;
and the `precip_variance` column inherits the same residual, on a path no shipped
config exercises (variance is flat at 1.0 everywhere), which is why D3 states the
conversion as a rule over columns rather than per column.
...
def axis_values(lookup_df, axis) -> pd.Series     # st_id -> axis value, refuses per D16/D17
def axis_caption(lookup_df, axis) -> str          # §5.5
def join_axes(indicators_df, lookup_df, surface) -> tuple[pd.DataFrame, pd.DataFrame]
```
...
`join_axes` returns `(surface_df, baseline_df)`: indicator rows whose `st_id` is
in the lookup, with the two derived axis columns attached, and — separately — the
rows whose `st_id` is absent from the lookup, which is exactly `st_0`. That is S6
implemented as a partition rather than as a documented convention, and it is why
D4's omission of `st_0` from the lookup is load-bearing rather than tidy.
...
1. **The library owns both reads.** `read_indicators(indicators_path)` sits beside
   ...
   **normalises both key columns to zero-padded strings** at
   `index_width(ST_NUM)` before partitioning, so a caller who loaded the frame
   some other way is repaired rather than silently mis-partitioned.
2. **`join_axes` asserts its partition and raises `BaselinePartitionError`**
   when either half is wrong: the set of `st_id` absent from the lookup must be
   **exactly** the baseline token `{index_width-padded 0}`, and `surface_df` must
   be non-empty. Both are cheap and both are checks the pre-change code did not
   need, because the axis was a column on the row.
...
1. **No in-repo *rule* consumes a response surface**, and the one in-repo
   consumer that exists can import the library directly. WF3 has no plotting rule
   ...
   (lines 683-685), plus two narrative sites (line 352, lines 481-482). Corrected,
   the reason gets **stronger**, not weaker: a notebook that can `import
   blueearth_cst.shared.surface_axes` is an argument *for* the library and against
   a rule. See §8 step 6 — the notebook is rewritten in the migration commit, not
   deferred, and it becomes the design's worked example of `join_axes` +
   `axis_caption`.
...
2. **If the varying set is empty, the axis is degenerate (D19).** Neither the
   ...
   declaration is **admitted** and returns the constant for those months; the
   default is all twelve. This is what keeps a temperature-only stress test legal
   on its precip axis — a one-dimensional design, not a malformed one.
...
All eight caption cases and both hazard cases were executed against `np.linspace`
member matrices; the rendered captions are in §9. (v1 said "six cases": the
degenerate row was one case in the §5.5 table and three rows in D19's, which §5.8
now reconciles as 4 / 4b / 4c.)
...
>   month)` sort order; `st_id` as zero-padded TEXT at the filename's width;
>   `st_0` ABSENT; the additive-vs-percent column semantics.
...
checked as text, since a validator that reads the frame with inferred dtypes would
itself be subject to the `01` -> `1` hazard the contract names. `ST_NUM` reaches
...
> `csthelpers`) and re-implement from this text. So everything an implementer
> needs is here, including the tolerances and the edge cases.
>
...
> Return the constant and mark the axis `degenerate`. `M` defaults to all twelve
> months; an explicit `M` is **admitted** and returns the constant for those
> months. Neither the subset rule nor the homogeneity rule of step 3 applies. A
> degenerate axis is an **annotation**, not a plot dimension: a consumer renders
> it in the caption rather than as an axis.
>
...
> from the design it describes; a derived one cannot. Over the classified months,
> with `H` the held months and `G` the distinct held levels:
>
> | case | condition | caption |
> |---|---|---|
> | 1 — uniform | `H` empty | `mean change over the year` |
> | 2 — some vary, rest unchanged | `G == {0}` | `mean change over JFM; Apr–Dec unchanged` |
> | 3 — rest held at one offset | `\|G\| == 1`, non-zero | `mean change over JFM; Apr–Dec held at -20%` |
> | 3b — rest held at several offsets | `2 <= \|G\| <= 3` | `mean change over JFM; Apr–Sep held at -20%; Oct–Dec held at -10%` |
> | 3c — more than three held levels | `\|G\| > 3` | `mean change over JFM; remaining months held at declared monthly offsets` |
...
> | 4c — degenerate, several levels | varying set empty, `\|G\| > 1` | `held at declared monthly offsets` |
>
...
> the unit of `variable` (`+3 °C`, `-20%`). The cap at three held groups is a
> legibility rule, not a correctness one: beyond three, the honest statement is
> that the held pattern is not summarisable and the reader should look at the
> lookup.
>
...
> baseline" rather than as an empty result. An implementation therefore **asserts
> its partition**: the set of `st_id` present in the tables but absent from the
> lookup must be **exactly** the baseline token, and the surface partition must be
> non-empty. Either violated is an error, not a shape.
>
...
as a named risk (R9). It would become preferable the moment WF3 grows its own
...
2. **Indicator values move by at most one `float64` ulp of the perturbation
   level, and are expected identical within the baseline comparator's
   tolerance.** v1 claimed the forcing was bit-identical; D25 measures that it is
   not, in general, and cannot be made so. The grid quantization is preserved, so
   the re-recorded baseline differs from the current one in the **column set** and
   at most in the last bits of the reconstructed multiplier. Falsifier:
   `compare_indicator_table` reporting a numeric failure — as opposed to the
   expected structural column-set mismatch — between a pre-change and a
   post-change run of `snake_config_baseline.yml`, **whose grid levels are in
   D25's exactly-invertible set**, so on that config the tolerance is not the
   thing being leaned on.
...
- **Step 4 — the library and its tests.** `shared/surface_axes.py` (with D28's
  ...
  `run_stress_test.smk`. **Plus `tests/test_surface_axes.py`**, which v1 omitted
  ...
  four exception types, `DEFAULT_SURFACE`, `AXIS_COLUMN` — and none of the four
  ...
  an ungated file. V5–V11 map onto named functions there (§9), and the
  `np.linspace` member matrices and rendered captions in §9's E10 block are
  promoted from run evidence into its fixtures.
...
- **Step 6 — docs, seeds and the notebook.**
  ...
  **`docs/notebooks/Climate Stress Test.ipynb` is rewritten in this commit** (R6),
  ...
  groups on the retired columns, so it raises `KeyError` after step 3. The rewrite
  is `surface_axes.read_lookup` + `read_indicators` + `join_axes`, grouping on the
  derived columns and rendering `axis_caption`, and it becomes **the design's
  worked example** — the one in-repo demonstration that the corrected axis is
  obtainable at all. The two remaining notebooks and the re-render pass stay with
  `t2608132100`.
...
> including **`tests/test_surface_axes.py`** (new).
>
...
| V11 | Captions match the **eight** cases | rendered caption != the §5.5 / §5.8 table | `test_surface_axes.py::test_caption_cases` |
...
| **V15** | **WG-2 validates the lookup, not a deleted file** | `validate_wg2` green on a `12 × ST_NUM` lookup, or green on a table containing an `st_0` row, or green on a `(st_id, month)` grid with a gap | `test_interchange_contracts.py` (rewritten `_wg2_good()` + a failing twin per clause) |
...
| **V17** | **The R filter yields exactly twelve ordered months** | `impose_climate_change.R` proceeding with `nrow != 12` or `month != 1:12` after the filter (D29) | R-side `stop()`; exercised by a WF3 run on the rapid config |
| **V18** | **`join_axes` refuses a mis-keyed partition** | `join_axes` returning an empty `surface_df`, or classifying a non-baseline `st_id` as baseline, instead of `BaselinePartitionError` (D28) | `test_surface_axes.py::test_partition_postcondition` |
...
| **V20** | **A non-round grid's forcing is reconstructed within the stated bound** | reconstructed multiplier differing from the `float32` level by more than one `float64` ulp on a 0.6–1.4 `step_num: 3` grid (D25) — the case the shipped configs cannot exercise | `test_prepare_cst_parameters.py` |
...
| gate | verdict |
|---|---|
| `pytest tests/test_prepare_cst_parameters.py`, `test_export_wflow_results.py`, `test_interchange_contracts.py`, `test_stress_test_grid.py`, **`test_surface_axes.py`** (new), **`test_check_baseline_indicator.py`**, **`test_check_baseline_scope.py`** | runnable — the narrow tier. The last three were missing from v1's list: `surface_axes` had no owning test file at all, and the two `check_baseline` files pin the retired seven-column header by literal, so they pass while asserting a dead contract (R10) |
