...
- P4 — Fingerprint and experiment lifecycle — [`phase-4-fingerprint-task-brief.md`](phase-4-fingerprint-task-brief.md) — **PARTIAL 2026-08-04; merged.** Report: [`phase-4-report.md`](phase-4-report.md). Commits 1–3 (the model fingerprint) complete: pointer-derived digest, per-experiment `model_reference.yml`, and a drift guard that fires before simulation. **Commit 4 (experiment ID collisions + config immutability) NOT DONE** — its checklist items 5–6 diverge from the code in four ways (a default name that appears nowhere, a file nothing writes, atomicity with no reservation step, and a home outside permitted scope). **Amended brief ready: [`phase-4-commit-4-task-brief.md`](phase-4-commit-4-task-brief.md) — four owner decisions listed at its top must be ruled before implementation**
